            ...
            <div class="max-w-7xl mx-auto space-y-10">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 border-l-[10px] border-l-blue-600 hover:shadow-xl transition-all">
                        <p class="text-slate-400 text-xs font-black uppercase tracking-widest mb-4">មុខវិជ្ជាបង្រៀន</p>
                        <div class="flex flex-wrap gap-2">
                            <?php if (mysqli_num_rows($res_subjects) > 0): ?>
                                <?php while($s = mysqli_fetch_assoc($res_subjects)): ?>
                                    <span class="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl font-black text-sm uppercase italic">
                                        <?= htmlspecialchars($s['subject_name']) ?>
                                    </span>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <span class="text-slate-300 font-bold italic">មិនទាន់មានទិន្នន័យ</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 border-l-[10px] border-l-purple-500 hover:shadow-xl transition-all">
                        ...
                        <h3 class="text-4xl font-black text-slate-800 mt-4 italic"><?= $count_students ?> <span class="text-lg text-slate-400">នាក់</span></h3>
                    </div>
                    ...
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 border-l-[10px] border-l-emerald-500 hover:shadow-xl transition-all">
                        ...
                        <h3 class="text-4xl font-black text-slate-800 mt-4 italic"><?= number_format($att_rate, 1) ?>%</h3>
                    </div>

                </div>

                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 bg-white rounded-[3rem] shadow-sm border border-slate-100 p-10 min-h-[250px] flex flex-col justify-center">
                         ...
                         <div class="flex items-center gap-4 text-slate-400 italic">
                             ...
                             <p>មិនទាន់មានការជូនដំណឹងថ្មីសម្រាប់ថ្ងៃនេះទេ...</p>
                         </div>
                    </div>
                    ...
                    <div class="bg-slate-900 rounded-[3rem] shadow-2xl p-10 flex flex-col items-center justify-center text-center text-white group overflow-hidden relative">
                         <div class="relative z-10 w-full">
                            <h3 class="font-black text-xl uppercase italic mb-6 tracking-widest">ស្រង់វត្តមានសិស្ស</h3>
                            <a href="attendance.php" class="block w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg active:scale-95 text-xs">
                                ចាប់ផ្តើមឥឡូវនេះ
                            </a>
                         </div>
