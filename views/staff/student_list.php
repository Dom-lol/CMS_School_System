...
$query = "SELECT s.*, u.full_name, u.username FROM students s 
          JOIN users u ON s.student_id = u.username 
          WHERE 1=1";

if ($search) {
    $query .= " AND (u.full_name LIKE '%$search%' OR s.student_id LIKE '%$search%')";
}
if ($class_filter) {
...
<main class="flex-1 p-8 bg-gray-50 min-h-screen">
    <div class="mb-8 flex justify-between items-center">
        ...
        <a href="add_student.php" class="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition flex items-center">
            <i class="fas fa-user-plus mr-2"></i> បន្ថែមសិស្សថ្មី
        </a>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-8">
        <form method="GET" class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="<?php echo $search; ?>" placeholder="ឈ្មោះ ឬអត្តលេខ..." 
            ...
            <a href="student_list.php" class="bg-slate-100 text-slate-500 px-6 py-3 rounded-xl font-bold hover:bg-slate-200 transition">
                Reset
            </a>
        </form>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                    ...
                    <th class="px-6 py-4 text-slate-600 font-bold text-xs uppercase">ឈ្មោះពេញ</th>
                    <th class="px-6 py-4 text-slate-600 font-bold text-xs uppercase">ភេទ</th>
    ...
    <?php else: ?>
        <tr>
            <td colspan="5" class="px-6 py-12 text-center text-slate-400 italic">មិនមានទិន្នន័យសិស្សឡើយ</td>
        </tr>
    <?php endif; ?>
</tbody>
        </table>
